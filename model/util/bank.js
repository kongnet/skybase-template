...
async function req (url = '', isJson = false) {
...
function getLocalCardType (codeNo) {
  for (let i = 0; i < bankcardList.length; i++) {
    for (let d = 0; d < bankcardList[i].patterns.length; d++) {
      let item = bankcardList[i].patterns[d]
      if (item.reg.test(codeNo)) {
        return {
          cardType: item.cardType,
          cardTypeCn: cardTypeMap[item.cardType],
          bank: bankcardList[i].bankCode,
          bankCn: bankCodeMap[bankcardList[i].bankCode],
          key: codeNo
        }
      }
    }
...
async function getAliCardType (codeNo) {
  let r = (
    await req(
      'https://ccdcapi.alipay.com/validateAndCacheCardInfo.json?cardBinCheck=true&cardNo=' +
        codeNo,
      true
    )
  ).data
  if (r.messages.length === 0) {
    r.cardTypeCn = cardTypeMap[r.cardType]
    r.bankCn = bankCodeMap[r.bank]
  }
  return r
}

async function getCardType (codeNo) {
  let r = getLocalCardType(codeNo)
  if (r.bank) {
    return r
  } else {
    r = await getAliCardType(codeNo)
    return r
  }
}
...
/*
async function load () {
  let r = await getCardType('6222005865412565805')
  console.log(r)
}
load()
银行卡位数必须是15到19位
